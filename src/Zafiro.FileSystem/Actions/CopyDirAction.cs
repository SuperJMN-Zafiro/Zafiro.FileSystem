...
public class CopyDirAction : IAction<LongProgress>
{
    private readonly CompositeAction compositeAction;
    
    private CopyDirAction(CompositeAction compositeAction)
    {
    ...
    public static async Task<Result<CopyDirAction>> Create(IZafiroDirectory source, IZafiroDirectory destination)
    {
        ...
        return action.Map(compositeAction => new CopyDirAction(compositeAction));
    }
}